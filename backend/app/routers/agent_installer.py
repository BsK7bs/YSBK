...
async def download_installer(
    ...
    if AGENT_EXE_PATH.exists():
        ...
        install_cmd_bytes = (
            ...
            "\r\n"
            "REM ------------------------------------------------------------------\r\n"
            "REM  PREP PHASE — always linear, always echoing.\r\n"
            "REM\r\n"
            "REM  This block replaces an older parenthesized cleanup dance that:\r\n"
            "REM    * hid `if (...)` blocks whose ECHO output was buffered by\r\n"
            "REM      cmd.exe until the block finished, giving the console a\r\n"
            "REM      long silent stretch that LOOKED like a hang;\r\n"
            "REM    * ran `taskkill /F /IM PythonService.exe` which is unsafe\r\n"
            "REM      on machines where OTHER (non-DigitalTwin) Python services\r\n"
            "REM      also run under that host name;\r\n"
            "REM    * ran a takeown/icacls/reset tree walk on a freshly created\r\n"
            "REM      empty %INSTALL_ROOT% (5-30 s silent stall for zero benefit);\r\n"
            "REM    * had a 16-second retry loop inside an `if exist` block\r\n"
            "REM      whose ECHO was buffered so users saw NOTHING for those\r\n"
            "REM      16 seconds.\r\n"
            "REM\r\n"
            "REM  The frozen installer .exe already stops/starts its own service,\r\n"
            "REM  copies files with retries, and creates the destination folder.\r\n"
            "REM  install.cmd only needs to:\r\n"
            "REM    (a) stop + delete any stale service registration so the\r\n"
            "REM        installer can re-register from scratch;\r\n"
            "REM    (b) kill the two OWNED processes (agent.exe / uninstaller.exe)\r\n"
            "REM        - NEVER PythonService.exe, it's a shared service host;\r\n"
            "REM    (c) best-effort remove the previous install folder;\r\n"
            "REM    (d) pair via HTTP + write the DPAPI credential;\r\n"
            "REM    (e) hand off to the installer with --no-pair --silent.\r\n"
            "REM\r\n"
            "REM  EVERY numbered step echoes BEFORE and AFTER its work so a hang\r\n"
            "REM  is trivially localisable (the last visible line = the step\r\n"
            "REM  that's stuck).\r\n"
            "REM ------------------------------------------------------------------\r\n"
            "\r\n"
            "echo [1/5] Stopping any existing DigitalTwinAgent Windows service ...\r\n"
            ">nul 2>&1 sc stop DigitalTwinAgent\r\n"
            ">nul 2>&1 sc delete DigitalTwinAgent\r\n"
            "echo       done.\r\n"
            "\r\n"
            "echo [2/5] Terminating any leftover agent executables (max ~5 seconds) ...\r\n"
            "REM  IMPORTANT: `taskkill /F /IM name.exe` has NO built-in timeout\r\n"
            "REM  and blocks INDEFINITELY when the target process is stuck in a\r\n"
            "REM  hung / STOP_PENDING / deadlocked state (documented Windows\r\n"
            "REM  limitation). Older versions of this script called taskkill\r\n"
            "REM  directly and users reported the console freezing forever at\r\n"
            "REM  this step on machines where a prior install had left agent.exe\r\n"
            "REM  in a non-responsive state.\r\n"
            "REM\r\n"
            "REM  Fix: launch a background PowerShell process (`start /B`) that\r\n"
            "REM  does the process cleanup with -ErrorAction SilentlyContinue,\r\n"
            "REM  then unconditionally sleep 5 seconds (`ping -n 6 127.0.0.1`\r\n"
            "REM  is more portable than `timeout /t 5` which can misbehave when\r\n"
            "REM  stdin is redirected). This HARD-CAPS the step at 5 seconds\r\n"
            "REM  regardless of whether PowerShell / Defender / SCM cooperate.\r\n"
            "REM  If a process is truly unkillable it will surface as a\r\n"
            "REM  'file in use' error from the installer .exe a few seconds\r\n"
            "REM  later, with a clear message telling the user to reboot.\r\n"
            "start /B /MIN \"\" powershell -NoProfile -WindowStyle Hidden -Command \"$ErrorActionPreference='SilentlyContinue'; foreach ($n in @('agent','DigitalTwinAgent','uninstaller')) { Get-Process -Name $n | Stop-Process -Force }\" >nul 2>&1\r\n"
            ">nul ping -n 6 127.0.0.1\r\n"
            "echo       done.\r\n"
            "\r\n"
            "echo [3/5] Cleaning previous install directory (if any) ...\r\n"
            "if exist \"%INSTALL_ROOT%\\agent.exe\" >nul 2>&1 del /F /Q \"%INSTALL_ROOT%\\agent.exe\"\r\n"
            "if exist \"%INSTALL_ROOT%\" >nul 2>&1 rmdir /S /Q \"%INSTALL_ROOT%\"\r\n"
            "if exist \"%INSTALL_ROOT%\\agent.exe\" (\r\n"
            "    echo       [!] Could not remove %INSTALL_ROOT%\\agent.exe.\r\n"
            "    echo           It is held open by another process. Please REBOOT\r\n"
            ...
            "echo       done.\r\n"
            "\r\n"
            "echo [4/5] Preparing install directory %INSTALL_ROOT% ...\r\n"
            "mkdir \"%INSTALL_ROOT%\" >nul 2>&1\r\n"
            ">nul 2>&1 icacls \"%INSTALL_ROOT%\" /grant *S-1-5-32-544:(OI)(CI)F /C /Q\r\n"
            ...
            "    echo             * Windows Defender's \"Controlled Folder Access\"\r\n"
            "    echo               is blocking Program Files modifications by\r\n"
            "    echo               unknown publishers. Temporarily disable it\r\n"
            "    echo               under Windows Security -^> Virus ^& threat\r\n"
            "    echo               protection -^> Manage ransomware protection.\r\n"
            "    echo             * A Group Policy / AppLocker / SRP rule\r\n"
            "    echo               restricts non-MSI writes to Program Files.\r\n"
            "    echo               Contact your IT administrator.\r\n"
            "    echo             * Your account, though marked \"Administrator\",\r\n"
            "    echo               is not actually elevated (rare - happens\r\n"
            "    echo               when UAC is set to \"Never notify\" with a\r\n"
            "    echo               broken filter). Log out, log back in, and\r\n"
            "    echo               rerun install.cmd.\r\n"
            "    echo.\r\n"
            "    pause\r\n"
            "    exit /b 5\r\n"
            ")\r\n"
            ">nul 2>&1 del /F /Q \"!__WRITE_TEST!\"\r\n"
            "echo       done. (write access confirmed)\r\n"
            "\r\n"
            "echo [5/5] Launching installer against %BACKEND_URL% ...\r\n"
            "\r\n"
            "REM ------------------------------------------------------------------\r\n"
