...
async def download_installer(
    ...
    if AGENT_EXE_PATH.exists():
        ...
        install_cmd_bytes = (
            ...
            "REM     not already an administrator. We probe elevation with\r\n"
            "REM     `fsutil dirty query %SYSTEMDRIVE%` instead of `net session`\r\n"
            "REM     because `net session` returns success even when we're a\r\n"
            "REM     non-elevated user IF the LanmanServer service is running,\r\n"
            "REM     and returns failure even for elevated admins if LanmanServer\r\n"
            "REM     is disabled (which is common on hardened workstations).\r\n"
            "REM     `fsutil dirty query` requires ERROR_ACCESS_DENIED unless the\r\n"
            "REM     caller has SeManageVolumePrivilege, which is granted only to\r\n"
            "REM     elevated Administrators. It has no side effects.\r\n"
            "REM\r\n"
            "REM     If not elevated, we ShellExecute-with-\"runas\" a small VBS\r\n"
            "REM     helper that re-launches THIS .cmd. VBS is available on every\r\n"
            "REM     supported Windows SKU going back to Windows 7 and needs no\r\n"
            "REM     PowerShell ExecutionPolicy juggling.\r\n"
            ">nul 2>&1 fsutil dirty query %SYSTEMDRIVE%\r\n"
            "if %errorlevel% neq 0 (\r\n"
            "    echo Requesting administrator privileges via UAC ...\r\n"
            "    set \"__ELEV_VBS=%TEMP%\\dt-elevate-%RANDOM%.vbs\"\r\n"
            "    >  \"!__ELEV_VBS!\" echo Set UAC = CreateObject(\"Shell.Application\")\r\n"
            "    >> \"!__ELEV_VBS!\" echo UAC.ShellExecute \"cmd.exe\", \"/c \"\"\" ^& WScript.Arguments(0) ^& \"\"\"\", \"\", \"runas\", 1\r\n"
            "    cscript //nologo \"!__ELEV_VBS!\" \"%~f0\"\r\n"
            "    del \"!__ELEV_VBS!\" >nul 2>&1\r\n"
            "    exit /b\r\n"
            ")\r\n"
            "\r\n"
            "echo.\r\n"
