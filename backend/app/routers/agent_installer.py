...
async def download_installer(
    ...
    if AGENT_EXE_PATH.exists():
        ...
        install_cmd_bytes = (
            ...
            ")\r\n"
            "REM --- Kill any stragglers holding agent.exe open. Do NOT use /T\r\n"
            "REM     because the /T tree-kill flag can wait on child processes\r\n"
            "REM     that are already exiting (Defender scan handles, WMI\r\n"
            "REM     provider callbacks, etc.) and produce a multi-second stall\r\n"
            "REM     with no visible output.\r\n"
            "echo [.] Terminating any lingering agent processes ...\r\n"
            "for %%P in (agent.exe uninstaller.exe DigitalTwinAgent.exe PythonService.exe) do >nul 2>&1 taskkill /F /IM %%P\r\n"
            "\r\n"
            "REM ------------------------------------------------------------------\r\n"
            ...
            "    echo     (this can take 10-30 seconds if the folder is large; please wait)\r\n"
            ...
            "echo [.] Ensuring install directory has write access ...\r\n"
            "REM  Skip the /T recurse flag on a fresh mkdir'd folder - it is\r\n"
            "REM  empty so recursion is pointless AND slower on some SKUs.\r\n"
            "if exist \"%INSTALL_ROOT%\\agent.exe\" (\r\n"
            "    >nul 2>&1 icacls \"%INSTALL_ROOT%\" /grant *S-1-5-32-544:(OI)(CI)F /T /C /Q\r\n"
            ") else (\r\n"
